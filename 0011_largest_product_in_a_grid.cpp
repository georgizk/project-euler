...
int main()
{
    static constexpr std::array<uint8_t, 400> GRID
    {
    ...
    auto mult = [](auto a, auto b, auto c, auto d) -> uint64_t
    {
        return static_cast<uint64_t>(a) * b * c * d;
    };
    // don't know of a very clever way to do this, other than maybe skipping 0s
    // left-right multiples
    for (std::size_t i{0}; i < 16; ++i) // column
    {
       for (std::size_t j{0}; j < 20; ++j) // row
       {
            largestProduct = std::max(largestProduct, mult(GRID[i + j*20], 
                GRID[i + 1 + j*20],
                GRID[i + 2 + j*20],
                GRID[i + 3 + j*20]));
       }
    ...
    // up-down multiples
    for (std::size_t i{0}; i < 20; ++i) // column
    {
       for (std::size_t j{0}; j < 16; ++j) // row
       {
            largestProduct = std::max(largestProduct, mult(GRID[i + j*20], 
                GRID[i + (j+1)*20],
                GRID[i + (j+2)*20],
    ...
    // diagonals leaning right
    for (std::size_t i{3}; i < 20; ++i) // column
    {
       for (std::size_t j{0}; j < 16; ++j) // row
       {
            largestProduct = std::max(largestProduct, mult(GRID[i + j*20], 
